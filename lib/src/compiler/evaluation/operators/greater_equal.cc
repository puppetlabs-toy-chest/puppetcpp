...
namespace puppet { namespace compiler { namespace evaluation { namespace operators {
    ...
    {
        explicit greater_equal_visitor(binary_operator_context const& context) :
            _context(context)
        {
        }
    ...
    value greater_equal::operator()(binary_operator_context const& context) const
    {
        return boost::apply_visitor(greater_equal_visitor(context), context.left(), context.right());
    }
