...
namespace puppet { namespace compiler { namespace evaluation { namespace functions {
    ...
    descriptor crit::create_descriptor()
    {
        functions::descriptor descriptor{ "crit" };

        descriptor.add("Callable", [](call_context& context) {
            auto& evaluation_context = context.context();
            return log(evaluation_context.node().logger(), *evaluation_context.current_scope(), logging::level::critical, context.arguments());
        });
        return descriptor;
    }
