...
namespace puppet { namespace compiler { namespace evaluation { namespace functions {
    ...
    descriptor debug::create_descriptor()
    {
        functions::descriptor descriptor{ "debug" };

        descriptor.add("Callable", [](call_context& context) {
            auto& evaluation_context = context.context();
            return log(evaluation_context.node().logger(), *evaluation_context.current_scope(), logging::level::debug, context.arguments());
        });
        return descriptor;
    }
