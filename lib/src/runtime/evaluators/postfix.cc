...
namespace puppet { namespace runtime { namespace evaluators {
    ...
    postfix_expression_evaluator::result_type postfix_expression_evaluator::operator()(ast::selector_expression const& expr)
    {
        ...
        for (size_t i = 0; i <  expr.cases.size(); ++i) {
            auto& selector_case = expr.cases[i];
        ...
        _position = expr.cases[*default_index].position();
        return _evaluator.evaluate(expr.cases[*default_index].result);
    }
    ...
    {
        runtime::dispatcher dispatcher(expr.method.value, expr.position());
