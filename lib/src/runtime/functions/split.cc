...
namespace puppet { namespace runtime { namespace functions {
    ...
    struct split_visitor : boost::static_visitor<value>
    {
        ...
        template <
            ...
            typename = typename enable_if<!is_same<Second, regex>::value>::type,
            typename = typename enable_if<!is_same<Second, type>::value>::type
        >
        result_type operator()(string const&, Second const& second) const
        {
            throw evaluation_exception(_context.position(1), (boost::format("expected String, Regex, or Type[Regexp] for second argument but found %1%.") % get_type(second)).str());
        }
